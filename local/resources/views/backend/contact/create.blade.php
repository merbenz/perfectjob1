...
<form action="{{ (empty($contact) ? url('manage/contact') : url('update/contact').'/'.$contact->id ) }}" method="POST" enctype="multipart/form-data" >
<!-- Alignments -->
		...
		<div class="panel panel-default">
			...
			{{ csrf_field() }}
				<div class="form-group">
					...
					<input type="file" name="img" class="form-control" value="{{ isset($contact)?$contact->img:'' }}"><br>
					@if(!empty($contact->img))
						<div style="text-align: center;">
							<img src="{{ asset('images/contact/').'/'.$contact->img }}" style="height:200px;"/>
						</div> 	 
					@endif
				</div>
			</div>
		...
		<div class="panel panel-default">
			...
			{{ csrf_field() }}
				<div class="form-group">
					...
					<input type="text" name="title_th" class="form-control" value="{{ isset($contact)?$contact->title_th:'' }}">
				</div>
				<div class="form-group">
                    <label>รายละเอียดเนื้อหา:</label>
                    <textarea class="form-control summernote" name="detail_th">{{ isset($contact)?$contact->detail_th:'' }}</textarea>
                </div>
...
<div class="page-body">
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-6">
		<div class="panel panel-default">
            ...
            <div class="form-group">
					<label>ชื่อหัวข้อ :</label>
					<input type="text" name="title_en" class="form-control" value="{{ isset($contact)?$contact->title_en:'' }}">
				</div>
				<div class="form-group">
                    <label>รายละเอียดเนื้อหา:</label>
                    <textarea class="form-control summernote" name="detail_en">{{ isset($contact)?$contact->detail_en:'' }}</textarea>
                </div>
            </div>
		</div>
	</div>
