...
<form action="{{ (empty($news) ? url('manage/news') : url('update/news').'/'.$news->id ) }}" method="POST" enctype="multipart/form-data" >
<!-- Alignments -->
		...
		<div class="panel panel-default">
			...
			{{ csrf_field() }}
				<div class="form-group">
					...
					<input type="file" name="img" class="form-control" value="{{ isset($news)?$news->img:'' }}"><br>
					@if(!empty($news->img))
						<div style="text-align: center;">
							<img src="{{ asset('images/news/').'/'.$news->img }}" style="height:200px;"/>
						</div> 								
					@endif
				</div>
			</div>
		...
		<div class="panel panel-default">
			...
			{{ csrf_field() }}
				<div class="form-group">
					...
					<input type="text" name="title_th" class="form-control" value="{{ isset($news)?$news->title_th:'' }}">
				</div>
				<div class="form-group">
                    <label>รายละเอียดเนื้อหา:</label>
                    <textarea class="form-control summernote" name="detail_th">{{ isset($news)?$news->detail_th:'' }}</textarea>
                </div>
...
<div class="page-body">
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-6">
		<div class="panel panel-default">
            ...
            <div class="form-group">
					<label>ชื่อหัวข้อ :</label>
					<input type="text" name="title_en" class="form-control" value="{{ isset($news)?$news->title_en:'' }}">
				</div>
				<div class="form-group">
                    <label>รายละเอียดเนื้อหา:</label>
                    <textarea class="form-control summernote" name="detail_en">{{ isset($news)?$news->detail_en:'' }}</textarea>
                </div>
            </div>
		</div>
	</div>
